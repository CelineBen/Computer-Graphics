...
public class Parser {
	...
	public static Scene createScene(Node dataNode) {
        ...
        if( ambientAttr != null ) {
        	...
        	float z = Float.parseFloat(s.next());
        	//float x = s.nextFloat();
            //float y = s.nextFloat();
            //float z = s.nextFloat();
        ...
        for ( int i = 0; i < nodeList.getLength(); i++ ) {
            ...
            if ( nodeName.equalsIgnoreCase( "material" ) ) {                
                ...
                Material.materials.put( material.name, material );
            	...
            	scene.root = Parser.createSceneNode(n);
            } 
        }    	
	...
	public static SceneNode createSceneNode(Node dataNode) {
		...
		if( scaleAttr != null ) {
            ...
            S.setElement(0,0,Double.parseDouble(s.next()));
            S.setElement(1,1,Double.parseDouble(s.next()));
            S.setElement(2,2,Double.parseDouble(s.next()));
            sceneNode.M.mul( S );
            s.close(); 
		}		
	...
	public static Light createLight(Node dataNode) {
        ...
        if( colorAttr != null ) {
        	...
        	float r = Float.parseFloat(s.next());
        	float g = Float.parseFloat(s.next());
        	float b = Float.parseFloat(s.next());
	...
	public static Camera createCamera(Node dataNode) {
        ...
        Node fovAttr = dataNode.getAttributes().getNamedItem("fov");
        if( fovAttr != null ) {
	...
	public static Material createMaterial(Node dataNode) {
		...
		Node refAttr = dataNode.getAttributes().getNamedItem("ref");
		if( refAttr  != null ) {
			material = Material.materials.get( refAttr.getNodeValue() );
		} else {
	    	...
	    	if( hardnessAttr != null ) {
	    		material.hardness = Float.parseFloat( hardnessAttr.getNodeValue() );
	    	}
		}
		return material;
	}
	...
	private static Material parseMaterial(Node dataNode, String tagName)
	{
    	...
    	for(int i = 0; i < nodeList.getLength(); i++)
    	{
			...
			if( name.equalsIgnoreCase(tagName) ) {
    			...
    			if( refNode != null ) { 
					material = Material.materials.get( refNode.getNodeValue() );
    			}
    			else {
